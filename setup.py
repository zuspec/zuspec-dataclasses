...
so the version can live in src/zuspec/dataclasses/__version__.py and reach setuptools
...
captures the dotted attribute path and leaves the closing brace stranded:
    ...
    version="zuspec.dataclasses.__version__.VERSION.dev<run-id>+gh"}
...
def _get_version():
    version_file = os.path.join(proj_dir, "src", "zuspec", "dataclasses", "__version__.py")
    glb = {}
