...
When creating ZuSpec-dataclasses models, follow these guidelines to ensure
maintainable, scalable, and clear device abstractions.

Choose the Right Abstraction Level
===================================

* **Start with logical MMIO**: Most models begin with register-level interfaces
* **Add operations when beneficial**: Create operation-level methods when they
  significantly simplify common use cases
* **Don't over-abstract**: Keep MMIO visible; operations should complement, not hide it
* **Match your verification goals**: Choose implementation detail based on what
  you're trying to verify

Organize Hierarchically
========================

* **Group related functionality**: Use Protocol classes to keep registers and
  events together
* **Respect hardware architecture**: Mirror the device's logical organization
* **Use Tuple for repetition**: When you have 3+ identical units, use
  ``Tuple[T, ...] = zdc.tuple(size=N)``
* **Separate global from per-unit**: Keep global control distinct from
  per-channel/port interfaces
* **Co-locate registers and events**: Events belong in the same Protocol as
  their related registers
...
.. code-block:: python
   ...
   class ChannelRegs(zdc.RegFile):
       csr : zdc.Reg[CSR] = zdc.field()
       addr : zdc.Reg[Addr] = zdc.field()
   
   @zdc.dataclass
   class Channel(Protocol):
       regs : ChannelRegs = zdc.field()
       done : zdc.Event = zdc.field()     # Event co-located with regs
   
   @zdc.dataclass
   class Device(Protocol):
       global_regs : GlobalRegs = zdc.field()
       channels : Tuple[Channel, ...] = zdc.tuple(size=4)  # Fixed-size array

Expose Logical Events
=====================

* **Identify all logical events**: Look for interrupt enable/source bits in specs
* **Don't just model physical interrupts**: Expose individual event sources,
  not just the aggregated interrupt pins
* **Place events correctly**: Events go in the same Protocol as their related registers
* **Support both polling and interrupts**: Logical events work for both access patterns

Event identification checklist:

* Interrupt enable bits (``INE_*``, ``IE``, ``*_IE``)
* Interrupt source/status bits (``INT_*``, ``IS_*``, ``*_IS``)
* Condition status bits (``DONE``, ``ERR``, ``BUSY``, ``READY``)
* Per-channel event replication
* Interrupt routing and masking architecture

Design for Usage
================

* **Think about access patterns**: Design interfaces for how they'll be used
* **Enable iteration**: Use Tuple fields so you can ``for ch in device.channels``
* **Provide clear paths**: ``device.channels[0].done`` beats ``device.events.ch0_done``
* **Support discovery**: Structure helps IDE autocomplete and documentation
* **Make programmatic access easy**: Fixed-size tuples enable list comprehensions

Maintain Consistency
=====================

* **Use consistent naming**: ``regs``, ``global_regs``, ``channels`` - not
  ``registers``, ``regs``, ``channel_array``
* **Follow conventions**: RegFile for registers, Protocol for interfaces,
  Event for logical events
* **Document the model**: Include docstrings explaining the device organization
* **Keep examples aligned**: If you have multiple similar devices, use similar
  structures

Scale Appropriately
===================

* **2-3 units**: Individual fields (``ch0``, ``ch1``) are acceptable
* **3+ units**: Use Tuple with ``zdc.tuple(size=N)``
* **Variable count**: Consider how the model will be parameterized for variants
* **Large arrays**: For very large arrays (e.g., 256 entries), ensure tuple
  overhead is acceptable

Avoid Anti-Patterns
===================

**❌ Don't flatten hierarchies**

.. code-block:: python

   # WRONG: All channels flattened
   class Device(Protocol):
       ch0_csr : zdc.Reg[CSR] = zdc.field()
       ch1_csr : zdc.Reg[CSR] = zdc.field()
       ch2_csr : zdc.Reg[CSR] = zdc.field()

**❌ Don't separate registers from events**

.. code-block:: python

   # WRONG: Events in separate namespace
   class Device(Protocol):
       regs : DeviceRegs = zdc.field()
       events : DeviceEvents = zdc.field()  # Disconnected

**❌ Don't use individual fields for many identical units**

.. code-block:: python

   # WRONG: Can't iterate, hard to extend
   class Device(Protocol):
       ch0 : Channel = zdc.field()
       ch1 : Channel = zdc.field()
       ch2 : Channel = zdc.field()
       ch3 : Channel = zdc.field()  # Use Tuple instead!

**❌ Don't hide MMIO behind operations prematurely**

.. code-block:: python

   # WRONG: No access to underlying registers
   class Device(Protocol):
       async def do_something(self): pass
       # Where are the registers? Can't debug or experiment!

By following these best practices, your models will be easier to understand,
maintain, and extend as your verification needs evolve.

============================================
Detailed Examples and Implementation Guidance
============================================

The following sections provide detailed examples and guidance for implementing
MMIO-level models with proper hierarchical organization.
