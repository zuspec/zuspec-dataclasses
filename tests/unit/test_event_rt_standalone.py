...
def test_event_rt_callback_sync():
    ...
    # Attempt to bind sync callback should raise TypeError
    try:
        evt.bind_callback(sync_callback)
        assert False, "Should have raised TypeError for sync callback"
    except TypeError as e:
        assert "must be async" in str(e)
        print(f"  → Correctly rejected sync callback: {e}")
    
    print("✓ test_event_rt_callback_sync passed")


async def test_event_rt_callback_async():
...
async def test_event_rt_callback_with_await():
...
async def test_event_rt_wait_and_callback():
...
async def test_event_rt_multiple_sets():
...
async def test_event_rt_callback_with_state():
